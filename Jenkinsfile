pipeline {
  ...
  tools {
	...
	stages {
        ...
        checkout scmGit(branches: [[name: "*/${params.BRANCH_NM}"]], extensions: [], userRemoteConfigs: [[credentialsId: 'git-cred', name: 'origin', url: 'https://github.com/kotagiriramachandra/RentA-Car-FrontEnd-Angular.git']])
				bat 'npm install'
				bat 'npm run build'        
		...
		stage ('Docker Image') {
			steps{
				script {
					bat 'docker build -t RentA-Car-FrontEnd-Angular:V1.0 .'
				}
			}
			post {
				success {
